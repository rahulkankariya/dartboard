  ...
  return (
    <div 
      ...
      style={{ 
        left: `${x}%`, 
        top: `${y}%`, 
        /* FIX: We added a CSS variable '--dart-scale' 
           Mobile: 0.6 | Laptop: 1.0
        */
        transform: `translate(-50%, -100%) rotate(${tilt}deg) scale(var(--dart-scale, 1))`,
        animation: 'dart-land 0.25s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards'
      }}
    >
      {/* Dynamic Scaling Wrapper */}
      <div className="flex flex-col items-center origin-bottom scale-[0.6] lg:scale-100">
      ...
      <style jsx>{`
        ...
        @keyframes dart-land {
          0% { 
            transform: translate(-50%, -400%) rotate(40deg) scale(calc(var(--dart-scale) * 2)); 
            opacity: 0; 
          }
          100% { 
            transform: translate(-50%, -100%) rotate(${tilt}deg) scale(var(--dart-scale)); 
            opacity: 1; 
          }
        }
      `}</style>
